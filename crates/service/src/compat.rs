...
const MANIFEST: &[MethodEntry] = &[
    ...
    // ─── ws ───────────────────────────────────────────────────────────
    // Since Tidepool 0.2.0: Surfpool (v1.1+) natively implements every
    // WS subscription method below. We run a thin reverse proxy to
    // Surfpool's WS port; no polyfill code remains.
    MethodEntry {
